...
# requested password). jlen is the length of the password.
    def genpool(self,jlen,jmin,jseed,jspc,jstr):
        ...
        random.seed(jseed or random.uniform(100.00,999.9)/random.uniform(4.0,9.9))
...
# Clean ambiguous and unwated characters from the genpool.
    def cleanpool(self,pool):
        ...
        return rem_sp
# Perform strict checks to ensure lower, upper, digit, and special characters.
    def strictpool(self,pool,strict):
        ...
        for chk in pool:
            if chk in string.punctuation:
                check4 = True
                break
        if any(checks == False for checks in (check1, check2, check3, check4)):
            check = False
        else: check = True
        return check
